...
// URL backend API
// Если пусто - используется относительный путь (для одного домена)
// Если указан - используется абсолютный URL (для разных доменов, например GitHub Pages)
const API_BASE_URL = window.API_BASE_URL || '';

// API функции
async function apiCall(endpoint, options = {}) {
    const initData = getInitData();
    // Если API_BASE_URL пустой, используем относительный путь
    // Если указан, используем абсолютный URL
    const baseUrl = API_BASE_URL || '';
    const apiUrl = baseUrl ? `${baseUrl}/api${endpoint}` : `/api${endpoint}`;
